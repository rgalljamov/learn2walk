...
MOD_REW_ET_10 = 'rew_et10'
MOD_DEV_ET = 'et_devi'
# steeper exp functions in individual reward functions and rew_et05
MOD_STEEP_REWS = 'steep_rews'

modification = mod([MOD_RELU, MOD_TORQUE_300, MOD_STEEP_REWS, MOD_REW_ET_10]) # mod([MOD_MINIMAL, MOD_REW_ET])
...
mio_steps = {HYPER_OWN:16, HYPER_PENG:6, HYPER_ZOO:2}[HYPERS]
...
info = f'hl{s(hid_layers)}_ent{int(ent_coef*1000)}_lr{lr_start}to{lr_final}_epdur{_ep_dur_in_k}_' \
       f'bs{int(batch_size/1000)}_imrew6121_gamma{int(gamma*1e3)}_prevbest_lr3_ent0'

# construct the paths
abs_project_path = dirname(dirname(dirname(__file__))) + '/'
save_path_norun= abs_project_path + \
                 f'models/baseline/{approach}/{modification}/{env_name}/{n_envs}envs/{algo}/{hyperparam}/{mio_steps}mio/' \
